...
# Read results file
with open("sweep_traces/results.txt", "r") as f:
    content = f.read()
...
# Write table to file
with open("sweep_traces/summary_table.txt", "w") as f:
    f.write("="*100 + "\n")
...
print(f"\nFull results saved to: sweep_traces/results.txt")
print(f"Summary table saved to: sweep_traces/summary_table.txt")
print(f"Trace files saved to: sweep_traces/<config_name>/")
EOF

echo ""
echo "Sweep complete!"
echo "Results: sweep_traces/results.txt"
echo "Summary: sweep_traces/summary_table.txt"
