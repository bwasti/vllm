...
class Qwen3Model(nn.Module):
    def __init__(
        ...
        *,
        ...
        # These are fed back autoregressively during inference
        self.hidden_state_projection = torch.nn.Linear(
            self.config.hidden_size,
            self.config.hidden_size,  # Stay in draft space, not target space
            bias=False,
        )
    ...
    def forward(
        ...
        # EAGLE3: Predict next hidden states for autoregressive speculation
        # These predicted hidden states will be fed back as input to predict future tokens
        predicted_next_hidden_states = self.hidden_state_projection(hidden_states)

        return hidden_states, predicted_next_hidden_states
