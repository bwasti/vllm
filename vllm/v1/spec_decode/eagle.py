...
class EagleProposer:
    def __init__(
        ...
        # Online EAGLE training components
        self.online_eagle_model = None
        self.training_buffer = None
        self.trainer = None
        self.training_iteration = 0  # Track iterations for sample matching
        if self.method == "online_eagle":
            # Will be initialized after model is loaded
            logger.info("Online EAGLE training will be initialized after model loading")
    ...
    def propose(
        ...
        # Online EAGLE: Complete pending samples from previous iteration
        # target_hidden_states are ground truth for what EAGLE predicted last iteration
        if self.training_buffer is not None and self.training_iteration > 0:
            # Complete samples from previous iteration
            prev_iteration = self.training_iteration - 1
            for idx in range(min(batch_size, target_hidden_states.shape[0])):
                if last_token_indices is not None and idx < len(last_token_indices):
                    token_idx = last_token_indices[idx].item()
                else:
                    # For first iteration or when indices not provided
                    continue

                # Get target hidden states and compute logits
                target_hidden = target_hidden_states[token_idx]
                with torch.no_grad():
                    target_logits = self.model.compute_logits(target_hidden.unsqueeze(0)).squeeze(0)

                # Complete the pending sample
        ...
        # Online EAGLE: Add pending samples for this iteration
        if self.training_buffer is not None:
            # Store EAGLE's predictions as pending samples
            for idx in range(batch_size):
                if idx < len(last_token_indices):
                    token_idx = last_token_indices[idx].item()

                    # Store the pending sample
                    self.training_buffer.add_pending_sample(
                        iteration=self.training_iteration,
                        token_idx=idx,
                        input_id=next_token_ids[idx].item(),
                        position_emb=positions[idx] if positions.dim() == 1 else positions[:, idx],
            ...
            if self.training_buffer.should_train():
                logger.info(
                    f"Training buffer full ({len(self.training_buffer)} samples), "
    ...
    def load_model(self, target_model: nn.Module) -> None:
        ...
        # Initialize online EAGLE training components if needed
        if self.method == "online_eagle":
            self._initialize_online_training()

    def _initialize_online_training(self):
        """Initialize online EAGLE training components."""
        from vllm.model_executor.models.online_eagle import create_online_eagle
        from vllm.v1.spec_decode.eagle_training_buffer import EagleTrainingBuffer
        from vllm.v1.spec_decode.eagle_trainer import EagleTrainer

        # Create trainable EAGLE model that references vLLM weights
        logger.info("Creating trainable Online EAGLE model...")
        self.online_eagle_model = create_online_eagle(self.model)

        # Create training buffer
        buffer_size = self.speculative_config.online_eagle_feedback_buffer_size
        sampling_rate = self.speculative_config.online_eagle_feedback_sampling_rate
        logger.info(
            f"Creating training buffer: buffer_size={buffer_size}, "
            f"sampling_rate={sampling_rate}"
        )
        self.training_buffer = EagleTrainingBuffer(
        ...
        # Create trainer
        learning_rate = self.speculative_config.online_eagle_learning_rate
        mse_loss_weight = self.speculative_config.online_eagle_mse_loss_weight
        logger.info(
            f"Creating trainer: lr={learning_rate}, "
            f"mse_weight={mse_loss_weight}"
        )
        self.trainer = EagleTrainer(
            online_eagle_model=self.online_eagle_model,
            learning_rate=learning_rate,
            mse_loss_weight=mse_loss_weight,
            kl_loss_weight=1.0,  # Always use KL loss
            gradient_clip_norm=1.0,
        )

        logger.info("✅ Online EAGLE training initialized successfully!")

    def _run_training_step(self):
        """Run a single training step on the buffered samples."""
        if self.trainer is None or self.training_buffer is None:
            return

        try:
            # Train on all samples in the buffer
            loss_dict = self.trainer.train_epoch(
                self.training_buffer,
                num_steps=None,  # Use all samples once
                batch_size=len(self.training_buffer),
            )
            ...
            if loss_dict:
                logger.info(
                    ...
                    f"mse_loss={loss_dict['mse_loss']:.4f}, "
                    f"kl_loss={loss_dict['kl_loss']:.4f}, "
                    f"grad_norm={loss_dict.get('grad_norm', 0):.4f}"
                )

                # Log EMA stats from trainer
                stats = self.trainer.get_stats()
                logger.info(
                    f"Training stats (EMA): "
                    f"steps={stats['train_steps']}, "
                    f"total_loss={stats['total_loss_ema']:.4f}, "
                    f"mse_loss={stats['mse_loss_ema']:.4f}, "
                    f"kl_loss={stats['kl_loss_ema']:.4f}"
                )
