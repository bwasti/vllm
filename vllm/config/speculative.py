...
class SpeculativeConfig:
    ...
    # Online EAGLE training configuration
    online_eagle_learning_rate: float | None = Field(default=1e-5, gt=0)
    """Learning rate for online EAGLE training. Only used when method is 'online_eagle'."""
    online_eagle_feedback_buffer_size: int | None = Field(default=256, gt=0)
    """Size of the training buffer for online EAGLE. Only used when method is 'online_eagle'."""
    online_eagle_feedback_sampling_rate: float | None = Field(default=1.0, gt=0, le=1.0)
    """Fraction of samples to collect for training (1.0 = collect all). Only used when method is 'online_eagle'."""
    online_eagle_mse_loss_weight: float | None = Field(default=1.0, ge=0)
    """Weight for MSE hidden state loss in online EAGLE training. Only used when method is 'online_eagle'."""
    ...
    def __post_init__(self):
        ...
        if self.method in ("ngram", "[ngram]"):
            ...
            if self.model is not None:
                ...
                if self.method in ("eagle", "eagle3", "online_eagle"):
                    pass
                # examples:
                ...
                if self.method in ("eagle", "eagle3", "online_eagle"):
                    ...
                    if isinstance(
                        ...
                        eagle_config = EAGLEConfig(
                            self.draft_model_config.hf_config,
                            method=self.method if self.method != "online_eagle" else "eagle",
                            model_type="eagle",
                        )
    ...
    def use_eagle(self) -> bool:
        return self.method in ("eagle", "eagle3", "online_eagle", "mtp")
